...
A repo could have the current version written in several places, e.g. the version file, throughout the README, tests, etc. Create a JSON file detailing all these locations - file paths plus a string containing the version for context. This will make sure only the correct strings are changed.

The version numbers must be X'd out: "x.x.x" or "X.X.X".  
...
steps:
  ...
  - name: Update version strings and CHANGELOG
    id: release-notes
    uses: mscwilson/changelog-creator@0.2.0
    with:
